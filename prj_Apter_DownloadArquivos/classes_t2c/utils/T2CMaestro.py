...
class T2CMaestro:
    ...
    def write_log(self, arg_strMensagemLog:str, arg_enumLogLevel:LogLevel=LogLevel.INFO, arg_enumErrorType:ErrorType=ErrorType.NONE):
        """
        ...
        if (arg_enumLogLevel != LogLevel.INFO): 
            print(arg_enumLogLevel.value + " - " + self.var_strReferencia + " - " + arg_strMensagemLog + " - " + arg_enumErrorType.value)
        else:
            print(arg_enumLogLevel.value + " - " + arg_strMensagemLog)

        #Criando e enviando log para o maestro
        var_jsonValues = {
            "Reference": self.var_strReferencia,
